...
# This function intakes a 1d list and returns it's derivative
def Derivative_1D_list(List_to_derivate):                                   # As input we accept a 1d list
    Derivative = np.diff(List_to_derivate)                                  # We then use the np.diff function to derivate the list
    Derivative = np.ndarray.tolist(Derivative)                              # We then convert the resulting array into a list to return a list
    Derivative.append(Derivative[-1])                                       # Finally we duplicate the last value to return a list with exactly the same values as the incoming one
    return Derivative                                                       # Return the Derivative list
...
class Timer:
    ...
    def Update_elapsed_time(self,kronos = False,Print_elapsed_time = False,Precise_time = False):                                                             # This method updates the elapse_time variable that takes the current time (time.time()) and this is subtracted from the start_time
        self.elapsed_time_precise = (time.time() - self.Start_time)                                                                                           # With it you can also print a string with the elapsed time in s or kronos and a precise time string if desired with all the decimals
        self.elapsed_time = int(round((time.time() - self.Start_time)))
        if kronos and Print_elapsed_time:
            if Precise_time:
                self.elapsed_time_precise_string = ('Elapsed time: '+str('{0:.6f}'.format(self.elapsed_time_precise/8640)+' kr. '))                         # It also provides a string print functionality in micronos (1000=8.6s) for easier elapsed time management
                print(self.elapsed_time_precise_string)
            else:
                self.elapsed_time_string = ('Elapsed time: ' + str('{0:.3f}'.format(self.elapsed_time_precise/8640)) + ' kr. ')                                     # It also provides a string print functionality in milikronos (1=8.6s) for easier elapsed time management
                print(self.elapsed_time_string)

        elif not kronos and Print_elapsed_time:
            if Precise_time:
                self.elapsed_time_precise_string = ('Elapsed time: '+str('{0:.6f}'.format(self.elapsed_time_precise)+' kr. '))                         # It also provides a string print functionality for easier elapsed time management
                print(self.elapsed_time_precise_string)
            else:
    ...
    def Update_progress(self,Current_step,Total_steps,Print_progress = False, kronos = False):                                        # This method more accurately tracks progress with both the steps and the time information it calculates the estimated time as well as the completion percentage
        self.elapsed_time = int(round((time.time() - self.Start_time)))
        ...
        if Print_progress:
            if kronos:
                self.Progress_string = (str('{0:.2f}'.format(self.Completion_percentage)) + ' % Elapsed time: ' + str('{0:.3f}'.format(self.elapsed_time/8640)) + ' kr, Estimated time: ' + str('{0:.3f}'.format(self.Estimated_time/8640)) + ' kr. ')  # We also provide print functionality for easier progress management
...
#     plt.ylabel('EQE /%')
#
#     # This is the test for the Derivative_1D_list function
#
...
#
#     fig = plt.figure()
#     plt.plot(Extract_Column(EQE_Data_1,0),Extract_Column(EQE_Data_1,1),label= 'EQE curve')
#     plt.plot(Extract_Column(EQE_Data_1, 0), Derivative_1D_list(Extract_Column(EQE_Data_1, 1)), label='EQE curve derivate')
#
#
#     plt.show()
